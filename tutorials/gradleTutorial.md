...
* **Tasks** are reusable blocks of logic.  For example, the stock `clean` simply deletes the project build directory. Tasks can be composed of other tasks or be dependent on another stock. 
...
1. Pull the branch named `gradle`. Merge it to the `master` branch. This will add the Gradle wrapper to your project.
   ...
   git merge gradle
   ```
1. Open the `build.gradle` file in an editor. Update the following code block to point to the main class (i.e., the one containing the `main` method) of your application. The code below assumes your main class is `seedu.eggventory.Eggventory`
   ```groovy
   application {
       mainClassName = "seedu.eggventory.Eggventory"
   }
   ```
...
:bulb: Simply run the command `gradlew {taskName}` in the terminal and Gradle will run the stock! Here are some example commands:
...
```groovy
shadowJar {
    archiveBaseName = eeggventoryntory
...
├─main
│  └─java
│     └─seeggventoryduke/Duke.java
└─test
    └─java
        └─seeggventoryduke/DukeTest.java
```
...
You can now write a test (e.g., `test/java/seeggventoryduke/DukeTest.java`) and run it with `gradlew test`.
