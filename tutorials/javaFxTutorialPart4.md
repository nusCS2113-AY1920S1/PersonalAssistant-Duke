...
1. Components are heavily dependent on each other:
   Why does `controlpanel.Main` need to know that `guicontroller.DialogBox` needs a `Label`? 
...
1. The code is untidy and long:
   ...
   The `controlpanel.Main` class attempts to do it all. 
...
<AnchorPane maxHeight="-Infinity" maxWidth="-Infinity" minHeight="-Infinity" minWidth="-Infinity" prefHeight="600.0" prefWidth="400.0" xmlns="http://javafx.com/javafx/8.0.171" xmlns:fx="http://javafx.com/fxml/1" fx:controller="guicontroller.MainWindow">
   <children>
      <TextField fx:id="userInput" layoutY="558.0" onAction="#handleUserInput" prefHeight="41.0" prefWidth="324.0" AnchorPane.bottomAnchor="1.0" />
...
1. Let’s explore the provided FXML files in Scene Builder. 
    ...
    Select `Open Project` > `src/main/resources/view/guicontroller.MainWindow.fxml`. Inspect each control and its properties.

   ![SceneBuilder opening guicontroller.MainWindow.fxml](assets/SceneBuilder.png)
...
1. On the left accordion, you can see that we have set the controller class to `guicontroller.MainWindow`. 
We will get to that later.
 
   ![Controller for guicontroller.MainWindow](assets/MainWindowController.png)

1. Let’s repeat the process for `guicontroller.DialogBox`.
   The main difference here is that guicontroller.DialogBox checks `Use fx:root construct` and _does not define a controller class_. 

   ![Settings for guicontroller.DialogBox](assets/DialogBoxController.png)
 ...
 */
public class guicontroller.MainWindow extends AnchorPane {
    @FXML
    ...
    private void handleUserInput() {
        ...
        dialogContainer.getChildren().addAll(
                guicontroller.DialogBox.getUserDialog(input, userImage),
                guicontroller.DialogBox.getDukeDialog(response, dukeImage)
        );
        userInput.clear();
    }
}
...
Notice how in `guicontroller.MainWindow`, we can invoke `TextField#clear()` on `userInput` and access its content just as we did in the previous example.
...
public class controlpanel.Main extends Application {
    ...
    public void start(Stage stage) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(controlpanel.Main.class.getResource("/view/guicontroller.MainWindow.fxml"));
            AnchorPane ap = fxmlLoader.load();
            Scene scene = new Scene(ap);
            stage.setScene(scene);
            fxmlLoader.<guicontroller.MainWindow>getController().setDuke(duke);
            stage.show();
        } catch (IOException e) {
            e.printStackTrace();
        }
...
For our custom `guicontroller.DialogBox`, we did not define a controller so let's create a controller for it.

**guicontroller.DialogBox.java**
```java
import java.io.IOException;
 ...
 */
public class guicontroller.DialogBox extends HBox {
    @FXML
    private Label dialog;
    @FXML
    private ImageView displayPicture;

    private guicontroller.DialogBox(String text, Image img) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(guicontroller.MainWindow.class.getResource("/view/guicontroller.DialogBox.fxml"));
    ...
    public static guicontroller.DialogBox getUserDialog(String text, Image img) {
        return new guicontroller.DialogBox(text, img);
    }

    public static guicontroller.DialogBox getDukeDialog(String text, Image img) {
        var db = new guicontroller.DialogBox(text, img);
        db.flip();
        return db;
    }
}
...
When we create a new instance of `guicontroller.DialogBox`, we set both the controller and root Node to `guicontroller.DialogBox`. 
From this point onwards we can interact with `guicontroller.DialogBox` as we have in the previous tutorials.

The last change that we have to make is to point our `controlpanel.Launcher` class in the right direction:
In `controlpanel.Launcher.java`
```java
//...    
Application.launch(controlpanel.Main.class, args);
//...
```
...
1. Convert `guicontroller.MainWindow` to use the `fx:root` construct.
1. Extend `guicontroller.MainWindow` to have a `Stage` as a root Node.
