...
Let's return to javacake and convert it to use FXML instead.
...
As part of the effort to separate the code handling javacake's logic and UI, let's _refactor_ the UI-related code to its own class.
...
public class MainWindow extends AnchorPane {
    ...
    public void setDuke(javacake d) {
        javacake = d;
    }
    ...
    private void handleUserInput() {
        ...
        String response = javacake.getResponse(input);
...
Let's create a new `Main` class as the bridge between the existing logic in `javacake` and the UI in `MainWindow`.
...
public class Main extends Application {
    ...
    public void start(Stage stage) {
        try {
            ...
            fxmlLoader.<MainWindow>getController().setDuke(javacake);
            stage.show();
        } catch (IOException e) {
            e.printStackTrace();
        }
