...
1. Components are heavily dependent on each other:
   Why does `trial.trial.Main` need to know that `DialogBox` needs a `Label`? 
...
1. The code is untidy and long:
   ...
   The `trial.trial.Main` class attempts to do it all. 
...
We will get to that later.
   ...
   ![trial.MovieHandler.Controller for MainWindow](assets/MainWindowController.png)
...
import javafx.scene.layout.VBox;
/**
 * trial.MovieHandler.Controller for MainWindow. Provides the layout for the other controls.
 */
...
Let's create a new `trial.trial.Main` class as the bridge between the existing logic in `Duke` and the UI in `MainWindow`.

**trial.trial.Main.java**
```java
@Override
 ...
 */
public class trial.trial.Main extends Application {
    ...
    public void start(Stage stage) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(trial.trial.Main.class.getResource("/view/MainWindow.fxml"));
...
```java
//...    
Application.launch(trial.trial.Main.class, args);
//...
```
