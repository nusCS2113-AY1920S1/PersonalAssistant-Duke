...
Let's return to mistermusik.Main and convert it to use FXML instead.
...
As part of the effort to separate the code handling mistermusik.Main's logic and UI, let's _refactor_ the UI-related code to its own class.
...
public class MainWindow extends AnchorPane {
    ...
    public void setDuke(mistermusik.Main d) {
        duke = d;
    }
...
Let's create a new `Main` class as the bridge between the existing logic in `mistermusik.Main` and the UI in `MainWindow`.
...
/**
 * A GUI for mistermusik.Main using FXML.
 */
public class Main extends Application {

    private mistermusik.Main duke = new mistermusik.Main();
