...
For controlpanel.Duke, there are two events that we want to respond to, namely the user pressing `Enter` in the `TextField` and left-clicking the `Button`. These are the `onAction` event for the `TextField` and the `onMouseClicked` event for the `Button`.
...
For this tutorial, we have two images `DaUser.png` and `DaDuke.png` to represent the user avatar and controlpanel.Duke's avatar respectively but you can use any image you want.
...
private String getResponse(String input) {
    return "controlpanel.Duke heard: " + input;
}
```
...
One additional benefit of defining a custom control is that we can add behavior specific to our `guicontroller.DialogBox`. Let’s add a method to flip a dialog box such that the image on the left to differentiate between user input and controlpanel.Duke’s output.
...
Congratulations! 
You have successfully implemented a fully functional GUI for controlpanel.Duke!  
