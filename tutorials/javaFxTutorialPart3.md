...
For control.Duke, there are two events that we want to respond to, namely the user pressing `Enter` in the `TextField` and left-clicking the `Button`. These are the `onAction` event for the `TextField` and the `onMouseClicked` event for the `Button`.
...
Let’s create our custom control `ui.DialogBox`:
```java
...
public class ui.DialogBox extends HBox {
    ...
    public ui.DialogBox(Label l, ImageView iv) {
        text = l;
        displayPicture = iv;
...
For this tutorial, we have two images `DaUser.png` and `DaDuke.png` to represent the user avatar and control.Duke's avatar respectively but you can use any image you want.
...
private void handleUserInput() {
    ...
    dialogContainer.getChildren().addAll(
            new ui.DialogBox(userText, new ImageView(user)),
            new ui.DialogBox(dukeText, new ImageView(duke))
    );
    userInput.clear();
}
...
private String getResponse(String input) {
    return "control.Duke heard: " + input;
}
```
...
## Iteration 3 – Adding custom behavior to ui.DialogBox

One additional benefit of defining a custom control is that we can add behavior specific to our `ui.DialogBox`. Let’s add a method to flip a dialog box such that the image on the left to differentiate between user input and control.Duke’s output.
...
public static ui.DialogBox getUserDialog(Label l, ImageView iv) {
    return new ui.DialogBox(l, iv);
}

public static ui.DialogBox getDukeDialog(Label l, ImageView iv) {
    var db = new ui.DialogBox(l, iv);
    db.flip();
    return db;
}
```
...
Now, we can go back to the `Main` class and change the event handler to use our new `ui.DialogBox`. 
...
private void handleUserInput() {
    ...
    Label dukeText = new Label(getResponse(userInput.getText()));
    dialogContainer.getChildren().addAll(
            ui.DialogBox.getUserDialog(userText, new ImageView(user)),
            ui.DialogBox.getDukeDialog(dukeText, new ImageView(duke))
    );
    userInput.clear();
}
```
...
Congratulations! 
You have successfully implemented a fully functional GUI for control.Duke!  
...
1. While the GUI looks similar to the mockup, there are still parts that need to be refined. Try your hand at some of these tasks:
   * Add padding between each ui.DialogBox
