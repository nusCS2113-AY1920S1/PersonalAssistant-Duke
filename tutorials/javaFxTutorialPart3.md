...
For controlpanel.FG, there are two events that we want to respond to, namely the user pressing `Enter` in the `TextField` and left-clicking the `Button`. These are the `onAction` event for the `TextField` and the `onMouseClicked` event for the `Button`.

For now, let’s have the application add a new `Label` with the text from the `TextField`. Update the `controlpanel.Main` class as follows. You'll need to add an `import javafx.scene.control.Label;` too.
```java
@Override
...
Let’s create our custom control `guicontroller.DialogBox`:
```java
...
public class guicontroller.DialogBox extends HBox {
    ...
    public guicontroller.DialogBox(Label l, ImageView iv) {
        text = l;
        displayPicture = iv;
...
We use the code in our main class just like any other control. Here are the steps to update the code to use the custom control in `controlpanel.Main.java`.
...
For this tutorial, we have two images `DaUser.png` and `DaDuke.png` to represent the user avatar and controlpanel.FG's avatar respectively but you can use any image you want.
...
```java
public class controlpanel.FG extends Application {
    // ...
...
private void handleUserInput() {
    ...
    dialogContainer.getChildren().addAll(
            new guicontroller.DialogBox(userText, new ImageView(user)),
            new guicontroller.DialogBox(dukeText, new ImageView(duke))
    );
    userInput.clear();
}
...
private String getResponse(String input) {
    return "controlpanel.FG heard: " + input;
}
```
...
## Iteration 3 – Adding custom behavior to guicontroller.DialogBox

One additional benefit of defining a custom control is that we can add behavior specific to our `guicontroller.DialogBox`. Let’s add a method to flip a dialog box such that the image on the left to differentiate between user input and controlpanel.FG’s output.
...
public static guicontroller.DialogBox getUserDialog(Label l, ImageView iv) {
    return new guicontroller.DialogBox(l, iv);
}

public static guicontroller.DialogBox getDukeDialog(Label l, ImageView iv) {
    var db = new guicontroller.DialogBox(l, iv);
    db.flip();
    return db;
}
```
...
Now, we can go back to the `controlpanel.Main` class and change the event handler to use our new `guicontroller.DialogBox`. 
...
private void handleUserInput() {
    ...
    Label dukeText = new Label(getResponse(userInput.getText()));
    dialogContainer.getChildren().addAll(
            guicontroller.DialogBox.getUserDialog(userText, new ImageView(user)),
            guicontroller.DialogBox.getDukeDialog(dukeText, new ImageView(duke))
    );
    userInput.clear();
}
```
...
Congratulations! 
You have successfully implemented a fully functional GUI for controlpanel.FG!  
...
1. While the GUI looks similar to the mockup, there are still parts that need to be refined. Try your hand at some of these tasks:
   * Add padding between each guicontroller.DialogBox
