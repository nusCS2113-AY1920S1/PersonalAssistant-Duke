...
For JavaFX.Main.Duke, there are two events that we want to respond to, namely the user pressing `Enter` in the `TextField` and left-clicking the `Button`. These are the `onAction` event for the `TextField` and the `onMouseClicked` event for the `Button`.

For now, let’s have the application add a new `Label` with the text from the `TextField`. Update the `JavaFX.Main` class as follows. You'll need to add an `import javafx.scene.control.Label;` too.
```java
@Override
...
In the mockup of the UI.UI, notice that the dialog boxes are composed of two different controls (`ImageView` and `Label`) and reused multiple times. In situations like this, it is often beneficial to create our own custom control.

Let’s create our custom control `JavaFX.DialogBox`:
```java
...
public class JavaFX.DialogBox extends HBox {
    ...
    public JavaFX.DialogBox(Label l, ImageView iv) {
        text = l;
        displayPicture = iv;
...
We use the code in our main class just like any other control. Here are the steps to update the code to use the custom control in `JavaFX.Main.java`.
...
For this tutorial, we have two images `DaUser.png` and `DaDuke.png` to represent the user avatar and JavaFX.Main.Duke's avatar respectively but you can use any image you want.
...
private void handleUserInput() {
    ...
    dialogContainer.getChildren().addAll(
            new JavaFX.DialogBox(userText, new ImageView(user)),
            new JavaFX.DialogBox(dukeText, new ImageView(duke))
    );
    userInput.clear();
}
...
private String getResponse(String input) {
    return "JavaFX.Main.Duke heard: " + input;
}
```
...
## Iteration 3 – Adding custom behavior to JavaFX.DialogBox

One additional benefit of defining a custom control is that we can add behavior specific to our `JavaFX.DialogBox`. Let’s add a method to flip a dialog box such that the image on the left to differentiate between user input and JavaFX.Main.Duke’s output.
...
public static JavaFX.DialogBox getUserDialog(Label l, ImageView iv) {
    return new JavaFX.DialogBox(l, iv);
}

public static JavaFX.DialogBox getDukeDialog(Label l, ImageView iv) {
    var db = new JavaFX.DialogBox(l, iv);
    db.flip();
    return db;
}
```
...
Now, we can go back to the `JavaFX.Main` class and change the event handler to use our new `JavaFX.DialogBox`. 
...
private void handleUserInput() {
    ...
    Label dukeText = new Label(getResponse(userInput.getText()));
    dialogContainer.getChildren().addAll(
            JavaFX.DialogBox.getUserDialog(userText, new ImageView(user)),
            JavaFX.DialogBox.getDukeDialog(dukeText, new ImageView(duke))
    );
    userInput.clear();
}
```
...
Congratulations! 
You have successfully implemented a fully functional GUI for JavaFX.Main.Duke!  
...
1. While the GUI looks similar to the mockup, there are still parts that need to be refined. Try your hand at some of these tasks:
   * Add padding between each JavaFX.DialogBox
