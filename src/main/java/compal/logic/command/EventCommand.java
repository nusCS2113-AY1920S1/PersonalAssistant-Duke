...
public class EventCommand extends Command {
    ...
    public static final String MESSAGE_USAGE = "event\n\t"
        + "Format: event DESCRIPTION /date DATE... /start START_TIME /end END_TIME [/interval INTERVAL] "
        + "[/priority PRIORITY] [/final-date FINAL_DATE]\n\n\t"
        ...
        + "\"...\" means you can add multiple. e.g. DATE... means you can add 01/01/2019 02/01/2019\n\t"
        + "For DATE and FINAL_DATE: dd/mm/yyyy is the date format. e.g. 01/01/2000\n\t"
        + "For START_TIME and END_TIME: HHmm is the time format. e.g. 1740\n\t"
        + "For PRIORITY: must be \"low\" or \"medium\" or \"high\"\n\n"
    ...
    public String createAndAddEvent(boolean isInOneDay, TaskList taskList, Date startDate) {
        ...
        for (Task task : taskList.getArrList()) {
            if (task.getSymbol().equals("E")
                ...
                && task.getStringTrailingDate().equals(trailingDateString)
                && task.getStringStartTime().equals(startTime)
                && task.getStringEndTime().equals(endTime)
                && task.getDescription().equalsIgnoreCase(description)) {
                return (MESSAGE_REPEATED_EVENT.concat(task.toString() + "\n"));
            }
        }
        ...
        return MESSAGE_GREETING.concat(indivEvent.toString() + "\n");
    }
}
