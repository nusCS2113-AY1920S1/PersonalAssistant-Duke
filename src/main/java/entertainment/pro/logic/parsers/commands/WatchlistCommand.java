...
public class WatchlistCommand  extends CommandSuper {
    ...
    private void addToWatchList() throws Exception {
        try {
            ...
            ((MovieHandler)(this.getUiController())).setGeneralFeedbackLabel("Please enter a valid command in the form of: \n"
    ...
    private void addDeadlineTask(String movie) throws IndexOutOfBoundsException {
        ...
        if (!WatchlistHandler.add(deadline)) {
            ...
            ((MovieHandler)this.getUiController()).setGeneralFeedbackLabel("No duplicates allowed");
        } else {
    ...
    private void executeIndexTaskDone()  {
        try {
            ...
            ((MovieHandler)(this.getUiController())).setGeneralFeedbackLabel("please enter a valid task number");
        }
    }
    ...
    private void executeNameTaskDone() {
        ...
        if (!WatchlistHandler.markMovieAsDone(movie, (MovieHandler)(this.getUiController()))) {
            ((MovieHandler)(this.getUiController())).setGeneralFeedbackLabel("please enter a movie in the watchlist");
        }
    }
    ...
    private void deleteFromWatchlist() {
        ...
        if (WatchlistHandler.removeFromWatchlist(mov, (MovieHandler)(this.getUiController()))) {
            ((MovieHandler) getUiController()).setGeneralFeedbackLabel("Successfully removed the movie from WatchList: " + mov);
        } else {
            ((MovieHandler) getUiController())
                    .setGeneralFeedbackLabel("Such a movie does not exist in your WatchList. Check your spelling?");
        }
    }
}