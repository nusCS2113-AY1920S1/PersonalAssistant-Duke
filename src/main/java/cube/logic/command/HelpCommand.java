...
public class HelpCommand extends Command {
	public static final String MESSAGE_SUCCESS =
			"Currently we support the following commands: (all command ignore cases)\n"
			+ "Manipulate food:\n"
			+ String.format("%1$-50s", "  'add <food name> -t <food type> -p <price> -s <stock> -e <expiry date>' ")
			...
			+ String.format("%1$-50s", "  'find -i <number> OR -n <food name> OR -t <food type> -sort <sort type>'")
					+ "find specific food using index/name/type, sort type is only available when finding by type, "
					+ "it can be expiry/name/stock\n"
			+ String.format("%1$-50s", "  'reminder -d <days to expiry> -s <low stock value>'")
					+ "show the list of food products that are low on stock and/or are approaching its expiry date\n"
			+ String.format("%1$-50s", "  'promotion <food name> -% <discount> -s <start date> -e <end date>'")
