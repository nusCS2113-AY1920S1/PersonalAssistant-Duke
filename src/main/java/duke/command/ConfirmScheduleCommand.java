...
public class ConfirmScheduleCommand extends Command {
    ...
    public ConfirmScheduleCommand(String userInputCommand) {
    ...
    public void execute(TaskList taskList, Ui ui, Storage storage) throws DukeException {
        ...
        if (userInputCommand.trim().equals(COMMAND_CONFIRMSCHEDULE)) {
            int i = 0;
        ...
        } else if (userInputCommand.trim().equals(COMMAND_CONFIRM)) {
            throw new DukeException(ERROR_MESSAGE_EMPTY_INDEX + MESSAGE_FOLLOWUP_EMPTY_INDEX);
        } else if (userInputCommand.trim().charAt(7) == ' ') {
