...
An option could be add to give a priority to the task : `todo DESCRIPTION prio INDEX`
...
An option could be add to give a priority to the task : `deadline DESCRIPTION /by DATE prio INDEX`
...
The find command will output a list of tasks whose description match with the keyword.
The list of matched tasks is ordered by a fuzzy matching feature.
...
To remind the first three tasks chronologically : `remind`
...
1. all the tasks will be showed, you have to choose a task INDEX
2. Depend on the type of task:
