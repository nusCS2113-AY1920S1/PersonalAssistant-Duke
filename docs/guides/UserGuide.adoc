...
[NOTE]
To enter task commands, please ensure you are in task mode. If not, see Section 4.1.2 for change mode
command using `flip`.
...
*Sorted by `time`*: deadline or events tasks with nearest date and time will be at the top of the task list.
...
[NOTE]
To enter email commands, please ensure you are in email mode. If not, see Section 4.1.2 for change mode
command using `flip`.
...
`addKeyword School -exp luminus -exp op`

`addKeyword ICM -exp ICM Career`
...
==== Linking Tasks and Email (coming in v2.0)
