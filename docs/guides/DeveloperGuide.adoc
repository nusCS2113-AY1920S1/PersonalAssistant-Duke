...
*Step 2*: `UI` component captures the input and passes to `Logic` component to parse the input. Below
...
** `input` is `email update 2`
** `optionList` is `[tag=Fun, tag=Project]`
...
** `responseMsg` here is "Tags added: [Project, Fun] to email: <title of email at index 2>"
