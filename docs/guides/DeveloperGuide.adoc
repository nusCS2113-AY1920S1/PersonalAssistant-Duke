...
task can be added into the task list quickly. This feature only works with `deadline` and `event` tasks.
The benefits of having this Natural Dates support are:
...
[caption="Figure N: "]
image::TaskParseNaturalDate.png[Natural Dates Support, 800]

The Natural Dates support is facilitated by four classes, namely `TaskCommandParseHelper`,
`CommandParseHelper`, `TaskParseNaturalDateHelper` and `Task`.
...
* Alternative 1 (chosen): Parse and return the time string as a pair that stores day as key and time as
value in a separate method.
** Pros: Easier to use the day and time in the pair in other methods without having to extract it from time
string again.
** Cons: Does not check whether day and time in the pair is valid. The day and time in the pair can be any
string. Have to call this method from another class which increase dependencies.
...
[caption="Figure N: "]
image::parseSortCommand.png[Parse Sort Command, 800]

Parsing of sort command is facilitated by two class, namely `TaskCommandParseHelper` and `TaskSortCommand`.
...
The sort command is facilitated by four class, namely `TaskSortCommand`, `Model`, `TaskList` and `UI`.
...
Aspect: When sorting of taskList occurs

* Alternative 1 (chosen): The task list is sorted whenever the GUI is updated.
** Pros: Task list will always be sorted according to how the user wants
** Cons: Sort command is executed even when the taskList does not need to be sorted
Current implementation invokes the `EmailTags#updateEmailTagList` on every user input to keep the tagMap and email list view in GUI updated, which increases the computational load.
...
** Cons: Harder to sort task list according to different sort type
