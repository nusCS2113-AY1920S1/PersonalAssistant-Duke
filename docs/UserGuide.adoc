...
Find tasks from the list based on the keyword(s) input by the user. +
Format: +
`*find* [KEYWORD(S)]`
...
==== Add a new entry to the shortcut: `create shortcut expense/income`
...
Users can look through, edit and remove limits in this mode. +
Format: +
`*limits*`
...
==== Add an entry from a shortcut to entries `add shortcut`
...
* *create shortcut expense/income* `create shortcut expense/income [DESCRIPTION] [AMOUNT] {TAG}` +
