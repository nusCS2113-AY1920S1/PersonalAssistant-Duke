...
*A*: The `data` folder containing your save data (called `save.txt` and `savedegree.txt`) is located one level above the jar file. Place the jar in another computer. Running it for the first time will create a `data` folder containing 2 empty save files. Overwrite these save files with the save files from your previous DEGREES.NUS `data` folder. +
...
*A*: This is deliberate as resizing the cli will break the formatting of the text displayed in the cli, particularly for commands `detail` and `command`.
...
*A*: No. We are prioritising support for other degrees, so linux support may come at a much later time.
