...
[TIP] Use the keyword `*list*` to list out the expenses and incomes to find the `*LIST NO.*` to be removed.
...
Format: +
* `modify [LIST NO.]`
[TIP] Try using the same method as `*remove*` to find `*[LIST NO.]*`.
