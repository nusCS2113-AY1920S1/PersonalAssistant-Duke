...
typing *`/create /profile /user_name USER_NAME /add /bank_name BANK_NAME /type SAVINGS`* and pressing kbd:[Enter]
...
* **`add`** `/bank_name DBB Multi-Currency Account /type savings /amount 8000` `: adds a new `DBB Multi-Currency Account`,
which has an initial amount of `8000` dollars.
* **`delete`** `/bank_name POSBkids`: deletes DBB Multi-Currency Account
...
** e.g. in `/add /bank_name BANK_NAME`, `BANK_NAME` is a parameter
which can be used as:
.. `/add /bank_name JunBank Savings Account /type savings`
.. `/add /bank_name Standard Bank Investment Account /type investment`
* Items in square brackets are optional parameters
** e.g. `/category [/category TAG]` can be used as:
.. `/transaction_add /amount 13.50 /category entertainment` (with optional `category` parameter)
.. `/transaction_add /amount 13.50` (without optional `category` parameter)
[WARNING]
====
...
[TIP]
====
Alternatively, you can access this User Guide document by clicking link:{repoURL}/main/blob/master/docs/UserGuide.adoc[here].
====
...
(e.g. `POBB Kids Savings Account`). | `BANK_NAME` should contain alphabets, spaces and dashes only
alphabet. +
