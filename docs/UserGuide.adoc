...
Reminds the user when there is only one day left for tasks/events. +
Format: `/remind`
...
Starts a exitable countdown that can be paused and resumed. +
...
Brings up the achievement page that shows the user the lists of achievements that have been unlocked as well as the total number of points the user has accumalated.

****
* Points can be earned from completing tasks, adding tasks and so on.
****

=== 3B.6 Avatar : `/avatar` +

This command will show user all information related to 
