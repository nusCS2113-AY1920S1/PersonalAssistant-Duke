...
== Features
*Command Format*
* Words in *bold* are the keywords that define the type of command to be executed. +
e.g. `*remove* [NO.]` `*remove* is the keyword that will execute the remove command`.
* Uppercase words in square brackets are the parameter to be inputed by the user. +
e.g. `*add expense* [AMOUNT] [DESCRIPTION] /on[DATE]`,`[AMOUNT]`, `[DESCRIPTION]` and `[DATE]` are parameters which the user can input as `*add expense* $5 dinner /on 11/11/2019`.
