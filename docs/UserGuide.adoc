...
You can add a `module` into `Oof` to manage `lesson`, `assignment` and `assessment` that you might have.
...
Adds a `module` with module code as "CS1010" and name as "Programming Methodology".
...
You can select a `module` in order to add `lesson`, `assignment` or `assessment` for that `module`.
...
You can add an `event` task with a scheduled starting and ending time. +
...
* `DESCRIPTION` of the `event` to be done can have multiple words, not just limited to single-word descriptions.
...
* Adding a *Semester*: `semester /add YEAR /name SEMESTER /from DD-MM-YYYY HH:MM /to DD-MM-YYYY HH:MM` +
...
* *Assignment*: `assignment DESCRIPTION /by DD-MM-YYYY HH:MM` +
e.g. `assignment Assignment 1 /by 31-10-2019 23:59`

Add an assessment task.

* *Assessment*: `assessment DESCRIPTION /from DD-MM-YYYY HH:MM /to DD-MM-YYYY HH:MM` +
...
[NOTE]
This feature runs on startup thus no input is needed from you.
