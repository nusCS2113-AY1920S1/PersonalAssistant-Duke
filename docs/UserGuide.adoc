...
==== List: `list`
View various lists of the stored resources based on the parameters entered.
...
. `list /date DATE`
. `list /date TAG`
* Only the names of items/room in the inventory can be entered
* Incorrectly entering the name of the resource will cause the UI to prompt the user again
* Dates should be entered in the DD/MM/YYYY HHmm format or in the natural date format like 'fri'

.*Examples*:
* `list /item pen`
* `list /room seminar room 1`
* `list /date 09/12/2019 1000`
* `list /tag TAG`
...
* *View upcoming deadlines*: `deadlines`
