...
`/add /recurexpenditure /amount AMOUNT /from BANK_NAME /in DAYS /description DESCRIPTION /category [CATEGORY]`

*Example*

`/add /recurexpenditure /amount 72.90 /from JunBank Savings Account /in 30 /description Telco Bills /category Bills`

Adds a recurring expenditure that deducts $`72.90` every `30` days to pay `Telco Bills` charged to `JunBank Savings Account`
categorised under the `Bills` category.

`/add /recurexpenditure /amount 2000 /from POBB Vickers Account /in 30 /description AAB Bonds /category Investments`

Adds a recurring expenditure that deducts $`2000` every `30` days to buy `AAB Bonds` charged to `POBB Vickers Account`
