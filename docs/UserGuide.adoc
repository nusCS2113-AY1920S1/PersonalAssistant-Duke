...
. Download the latest `OwlMoney.zip` link:{repoURL}/releases/latest[here].
. Copy the `OwlMoney.zip` file to the folder you want to use as the home folder for *OwlMoney*.
. Extract all contents in the zip file into the home folder, including the `data` directory which should be *empty*.
. Open a *Command Prompt* or *Powershell*, navigate to the home folder and type `java -jar ./OwlMoney-version number
.jar` to start *OwlMoney*.
*OwlMoney* should appear in a few seconds.
. Maximise the screen to enjoy the best experience of *OwlMoney*.
...
Adds a goal named GRAD TRIP which aims to save $`2000` by `30/12/2020` and tie it to a savings account named `JunBank
Savings Account`.
