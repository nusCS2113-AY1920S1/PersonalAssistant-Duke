...
.  Run the oof.command “java -jar oof.jar”. The application should load within a few seconds.
+
...
.  Refer to <<Features>> for details of each oof.command.

[[Features]]
== Features
In this section, the expected oof.command format will be introduced, and you can expect to learn the various commands you can use.
====
*Command Format*
...
*A*: Use the `viewCalendar` oof.command.
