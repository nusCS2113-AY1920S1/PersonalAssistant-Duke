...
Format: `reccuring DESCRIPTION`

* `Description` of the task to be done can have multiple words, not just limited to single word descriptions.

Example:

* `recurring math lecture` +
Adds a recurring task called `math lecture` that will be repeated every week.
...
=== View free time slots: `viewFree`
Gives a list of free time slots for the next 7 days.
...
=== Add estimated time taken: `estimate [coming soon in v2.0]`
...
=== Adding a task: `range [coming soon in v2.0]`
...
=== View two different calendars side-by-side: `viewDual [coming soon in v2.0]`
Transforms all current tasks into two calendar views, one for tutor tasks and one for student tasks. 

Format: `viewDual`

=== Export calendar: `export [coming soon in v2.0]`
