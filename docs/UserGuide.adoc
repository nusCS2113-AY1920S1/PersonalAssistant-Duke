...
. Download the latest `OwlMoney-v1.3.zip` link:{repoURL}/releases/latest[here].
. Copy the `OwlMoney-v1.3.zip` file to the folder you want to use as the home folder for *OwlMoney*.
. Extract all contents in the zip file into the home folder, including the `data` directory which should be *empty*.
. Open a *Command Prompt* or *Powershell*, navigate to the home folder and type `java -jar ./OwlMoney-v1.3.jar` to
start *OwlMoney*.
