...
#### `prev` - Display previous page of Patients [[home-prev]]
...
Format: `open _patient_id_ [-im[pression] [<impression index>]]`
...
Opens the new Treatment or Evidence item's context if `-g[o]` is specified. Relevant switches for various types are as follows.
