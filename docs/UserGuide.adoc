...
... The requested date must be a valid date, either in the format `DD/MM/YYYY HHmm`, or in the example format `Tuesday HHmm`, where `Tuesday` refers to the next instance of Tuesday from the current date.
