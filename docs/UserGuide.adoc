...
#### `open` - Open a Treatment (Investigation, Medicine or Plan) or Evidence (Observation or Result) item on this page

Format: `open ("<search string>"|-m[edicine] _string_or_idx_|-in[v(x|estigation)] _string_or_idx_|-p[lan] _string_or_idx_|-o[bservation] _string_or_idx_|-r[esult] _string_or_idx_)`
