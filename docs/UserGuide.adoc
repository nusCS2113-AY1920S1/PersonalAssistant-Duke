...
.  Copy the file to an empty folder you want to use as the home folder for your WordUp.
...
e.g. `search_begin w/d`
* *Search Synonym* : `search_syn w/WORD` +
e.g. `search_syn w/life`
* *Search Tag* : `search_tag [w/WORD]/[t/TAG]` +
e.g. `search_tag w/life`
