...
Compares between Industrial and Systems Engineering Environmental Engineering.
* `compare` +
...
There are 4 priority <LEVELS> and can be used for all custom tasks, including `event` and `deadline`: +
...
* `todo Go on a Bizzare Adventure` +
Creates a todo task with the description "Go on a Bizzare Adventure" with normal priority.
...
*A*: Install the app in the other computer and overwrite the empty data file it creates with the file that contains the data of your previous DEGREES.NUS folder. The save files should be called `save.txt` and `savedegree.txt`. +

*Q*: Why can't I find information regarding this degree? +
*A*: If your course is not offered by the Faculty of Engineering, chances are our program does not support it. We do intend to expand our app to eventually include all courses in NUS, but for now only some Enigneering degrees are supported.
