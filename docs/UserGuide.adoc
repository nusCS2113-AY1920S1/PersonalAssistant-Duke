...
#### `up` - Go up to next-higher context

Format: `up`

_Available in:_ <<Patient>>, <<Impression>>, <<Result>>, <<Observation>>, <<Investigation>>, <<Medicine>>, <<Plan>>

This will go to the context hierarchically above the user's context. In this case, it will go back to <<Home>>. From an <<Impression>>, it would come here, to the corresponding <<Parient>>.
