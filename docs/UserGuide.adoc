...
* Since there is a lot of fairly complex data to manage, I think the input system will have to be stateful - different commands mean different things based on the current context. 
...
Inherits: <<home-help,`help`>>, <<home-next,`next`>>, <<home-prev,`prev`>>, <<patient-back,`back`>>, <<patient-up,`up`>>, <<patient-report,`report`>>
...
#### `open` - Open a Treatment (Investigation, Medicine or Plan) or Evidence (Observation or Result) item listed on this page
...
If input without `_type_ _string_or_idx_` unctionally the same as <<patient-edit,`edit` for Patients>>, but with different possible switches.
