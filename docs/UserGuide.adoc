...
a| 
* `-n[ame] "<name>"`
* `-s[tatus] ("<status name>"\|<status idx>)`
* `-d[ose] "<dose>"`
* `-da[te] "<start date>"`
* `-du[ration] "<duration of course>"`
...
* `-sum[mary] "<summary>"`
* `-subj[ective]`
* `-obj[ective]` - default to objective observations
* `-p[riority] <priority idx>`
...
Functionally the same as <<patient-edit,`edit` for Patients>>, but with different possible switches.
