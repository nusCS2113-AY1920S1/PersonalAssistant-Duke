...
If you require a full list of possible commands that D2 understands, you can call the help command
...
=== Adding a degree programme into your list: `add`
...
[TIP]
A degree programme can have any number of tags (including 0)
...
=== Listing all degree programmes in your list : `list`

Shows a list of all degrees in the list currently. +
Format: `list`

=== Swapping degree programmes in the list: `swap`
...
Edits an existing degree programme into a new degree the task list. +
Format: `edit INDEX NEWDEGREE`

****
* Edits the degree at the specified `INDEX`. The index refers to the index number shown in the displayed person list. The index *must be a positive integer* 1, 2, 3, ...
