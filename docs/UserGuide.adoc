...
_Work in progress:_ If possible, we want to eliminate the requirement for having spaces to separate switch arguments from switches.
...
* `<Space>` -> Words in a command are separated by spaces. Spaces can be omitted except for the space after the command name and the space between a switch and its option.
...
The discharge report will contain all information being tracked regarding the Patient, and an optional discharge summary that can be specified in the command. In v1.x, Patients will simply be deleted after the report is generated. In v2.0, they will be archived.
