...
* _patient_id_ -> `(*-b[ed]* <bed number> | *-i[ndex]* <patient's index>)`
* _string_or_idx_ -> `(*-n[ame]* <name>) | *-i[ndex]* <index>`
...
Format: `open _string_or_idx_ (*-c[ritical]* | *-in[v(x | estigation)*] | *-im[pression]*)`
...
Format: `delete _string_or_idx_ (*-c[ritical]* | *-in[v(x | estigation)*] | *-im[pression]*)`
