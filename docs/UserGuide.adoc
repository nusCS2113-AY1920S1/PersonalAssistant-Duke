...
| 2 | Result Box | This provides you with information on whether a command is successful.
|====
...
Edits the amount in `DBB Vickers Account` to $`50 000`.
...
Edits the credit limit of `POBB Tomorrow Card` to $`10 000`.

* `/edit /card /name POBB Tomorrow Card /rebate 2.05`

Edits the cashback rate of `POBB Tomorrow Card` to `2.05`%.
...
Adds an expenditure that charged to `MYCARD` dated `28/09/2019` that costs `$1.20` with the description of `bubble
...
Edits expenditure `2` that was charged to `MYCARD` to a new description of `Uniwlo Shirt`.

* `/edit /cardexpenditure /from MYCARD /transno 5 /amount 3.50`

Edits expenditure `5` that was charged to `MYCARD` to an amount of $`3.50`
...
In this section, you'll be introduced to commands and parameters that help you to create financial goals to
...
Edits the name of the goals from `BTO at Punggol Downpayment` to `BTO at Tampines Downpayment`.

* `/edit /goals /name BTO at Punggol Downpayment /amount 27500`

Edits the goals of `BTO at Punggol Downpayment` amount to $`27500`.

* `/edit /goals /name BTO at Punggol Downpayment /by 11/11/2021`

Edits the goals of `BTO at Punggol Downpayment` to a new date of `11/11/2021`.
...
Edits the profile name to JUNNY
