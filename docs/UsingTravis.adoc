...
image::signing_in.png[Signing into Travis CI, width = 800]
+
...
image::review_and_add.png[Review and add, width = 800]
+
...
+
.Build pending for Travis
image::build_pending.png[Commit build, width = 800]
+
* As the build is run on a provided remote machine, we can only examine the logs it produces:
+
.Viewing the logs for Travis
...
+
.Generating a token
image::generate_token.png[Generate token, width = 800]
...
--
.Adding a token
image::travis_add_token.png[Travis add token, width = 800]
+
[NOTE]
