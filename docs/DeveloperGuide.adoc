...
<<<<<<< HEAD BakingHome's products can have two status: `ACTIVE` or `ARCHIVE`.
This feature allows user to view product with a given certain status, i.e. shows only products with an ARCHIVE status.
...
In particular `PREDICATE_SHOW_ARCHIVE_PRODUCTS` is as follow
...
**Step 2.** The user inputs `product filter -scope archive` to list all archived products. `UI` pass the input to `Logic`.
Logic then uses a few `Parser` class to extract layers of information out.
...
*Step 4.* `ModelManager#AddSale()` is called which in turn calls `BakingHome#AddSale()` and passes the created sale object into the `UniqueEntityList` of `Sale`. At the same time, the `FilteredSaleList` is updated with the predicate PREDICATE_SHOW_ALL_SALES which as the name suggests, is usd to display every sale entry.

==== Design Consideration

* Alternative 1 (current implementation): Store all sale entries in an observable list in BakingHome and keep a FilteredList in ModelManager.
...
=======
>>>>>>> dc1d96d0fa2b4b5280e1aad866e691c2c53c6d52
[appendix]
...
=======
. Adding a product with cost and price
.. Test case: `product add -name Cheese cake -cost 3.0 -price 5.9` +
>>>>>>> dc1d96d0fa2b4b5280e1aad866e691c2c53c6d52 Expected: A new product named `Cheese cake` is added.
Ingredient cost are $3 and Retail price is $5.9. Status is active.
Use `product show INDEX_OF_CHEESE_CAKE`
...
=== Filtering Products <<<<<<< HEAD
=======

. Filters to show only archived products >>>>>>> dc1d96d0fa2b4b5280e1aad866e691c2c53c6d52
...
<<<<<<< HEAD Expected: All products, regardless their status, are listed.
...
BakingHome provides basic Add, Edit, Delete operations for all entity.
The tables belows provides a few commands for you to try out.
...
=== Delete operation Delete operation is have the same syntax for all entity.
The table below use order as an example. `order`
...
.. Test case (corrupted data file): Change the content of data file at `data/baking.json` to "This file is damaged" and restart BakingHome. +
   Expected: BakingHome loads demo data >>>>>>> dc1d96d0fa2b4b5280e1aad866e691c2c53c6d52
