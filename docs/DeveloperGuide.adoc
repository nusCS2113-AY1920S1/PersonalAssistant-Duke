...
==== Implementation
The `ViewTrackerCommand` class extends `Command` class by providing functions to display a bar graph visualising the amount of time spent on each `Module` filtered by `Day`, `Week` and `All`.
...
*Step 1:* +
The user enters `start CS2113T Activity Diagram`. The `parse` method in `CommandParser` class is called to parse the user input to obtain the String `CS2113T Activity Diagram` as the `Task` that the user wants to start the Time Tracker timer for.

[WARNING]
`OofException` will be thrown is the user enters an invalid command.

*Step 2:* +
The `execute` method of `ViewTrackerCommand` class will...
...
** Alternatives Considered: +
other possible implementations that is less good
