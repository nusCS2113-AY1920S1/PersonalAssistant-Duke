...
=== What is *_OOF_*?
*_OOF_*, short for Outstanding Organisation Friend, is a command-line interface desktop application built to improve University Students' productivity and efficiency by reducing the need to manually organise tasks.

=== What is the purpose of this guide?
This guide aims to provide information for you, future contributors of *_OOF_* so that you can have an easy reference for understanding the features implemented in *_OOF_*.
...
[[LogicProductivityClassDiagram]]
.Structure of the productivity subpackage in the Logic Component
image::LogicProductivityClassDiagram.png[LogicProductivityClassDiagram]

The *_Class Diagram_* above illustrates the relationship between the individual components of the `productivity` subpackage in the `Logic` component.

[[LogicTaskClassDiagram]]
.Structure of the task subpackage in the Logic Component
image::LogicTaskClassDiagram.png[LogicTaskClassDiagram]

The *_Class Diagram_* above illustrates the relationship between the individual components of the `task` subpackage in the `Logic` component.

[[LogicSemesterClassDiagram]]
.Structure of the semester subpackage in the Logic Component
image::LogicSemesterClassDiagram.png[LogicSemesterClassDiagram]

The *_Class Diagram_* above illustrates the relationship between the individual components of the `semester` subpackage in the `Logic` component.

[[LogicOrganizationClassDiagram]]
.Structure of the organization subpackage in the Logic Component
image::LogicOrganizationClassDiagram.png[LogicOrganizationClassDiagram]

The *_Class Diagram_* above illustrates the relationship between the individual components of the `organization` subpackage in the `Logic` component.

[[LogicModuleClassDiagram]]
.Structure of the module subpackage in the Logic Component
image::LogicModuleClassDiagram.png[LogicModuleClassDiagram]

The *_Class Diagram_* above illustrates the relationship between the individual components of the `module` subpackage in the `Logic` component.

[[LogicLessonClassDiagram]]
.Structure of the lesson subpackage in the Logic Component
image::LogicLessonClassDiagram.png[LogicLessonClassDiagram]

The *_Class Diagram_* above illustrates the relationship between the individual components of the `lesson` subpackage in the `Logic` component.
...
Classes used by multiple components are in the `oof.commons` package.
...
[[TasksAffectingFree]]
.Example of tasks on the input date and deadlines due for Free Command. 
image::TasksAffectingFree.png[TasksAffectingFree]
...
*Step 1:* +
The user enters `tracker` command.
The `execute` method of `TrackerCommand` class reads and saves all `Tracker` objects stored in persistent storage, `tracker.csv` through the `readTrackerList` method in the `StorageManager` class.
...
* *Step 1c:* +
The `processLine` method of `StorageParser` class splits each line into its respective fields through the use of `,` delimiters before parsing and assigning them to the correct fields.
A new `Tracker` object will be created with the processed data and returned to the `dataToTrackerList` method.

* *Step 1d:* +
The `Tracker` object returned to `readTrackerList` will be added into the ArrayList `Tracker` objects and upon completing the entire `tracker.csv` file, the ArrayList will be returned to the `execute` method of `TrackerCommand`.
...
It will then be removed from the ArrayList before the `execute` method of `TrackerCommand` calls the `printTrackerDelete` method in the `Ui` class and updates `tracker.csv` by calling `writeTrackerList` method from `StorageManager` class.
...
*Step 5:* +
If the instruction is not `/view`, `/list` or `/delete`, the `execute` method of `TrackerCommand` will obtain the `TASK_INDEX` and `MODULE_CODE` given by the user
...
. OOF displays expiring tasks every time OOF is started.
...
. OOF lists the current tasks saved in the program and prompts the User to select the task to be deleted.
...
Expected: Acknowledgement message displayed regarding the successful addition of `Tutorial` for the selected `module`.
...
. Prerequisites: List all tasks using `list` command. List must not be empty.

. Test case: `tracker /start 1 CS2113T` +
Expected: A tracker is started for the first task on the list.
Details of the tracker will be displayed alongside current time and the amount of time spent on this task, if any, so far.

. Test case: `tracker /start 0 CS2113T` +
Expected: Error displayed due to invalid Task index.

. Other start tracker commands to try: `tracker /start abc`, `tracker /start -1` +
Expected: Error displayed due to invalid input.

==== Stopping Task Tracker
The `tracker /stop` command syntax is as such: `tracker /stop TASK_INDEX MODULE_CODE`.

. Prerequisites: List all tasks using `list` command. List must not be empty.
The corresponding Task Tracker must have started before this command is available.

. Test case: `tracker /stop 1 CS2113T` +
Expected: The tracker is stopped and marked as completed for the first task on the list.
Details of the tracker will be displayed alongside current time and the amount of time spent on this task, if any, so far.

. Test case: `tracker /stop 0 CS2113T` +
Expected: Error displayed due to invalid Task index.

. Other stop tracker commands to try: `tracker /stop abc`, `tracker /stop -1` +
Expected: Error displayed due to invalid input.
...
. Test case: `tracker /pause 1 CS2113T` +
Expected: The tracker is paused for the first task on the list.
Details of the tracker will be displayed alongside current time and the amount of time spent on this task, if any, so far.

. Test case: `tracker /pause 0 CS2113T` +
Expected: Error displayed due to invalid Task index.

. Other pause tracker commands to try: `tracker /pause abc`, `tracker /pause -1` +
Expected: Error displayed due to invalid input.

==== Viewing Task Tracker Diagram
The `tracker /view` command syntax is as such: `tracker /view PERIOD`.

. Prerequisites: List all trackers using `tracker /list` command. List must not be empty.

. Test case: `tracker /view day` +
Expected: A histogram will be displayed featuring all of the tracked tasks today sorted and grouped by module code.

. Test case: `tracker /view week` +
Expected: A histogram will be displayed featuring all of the tracked tasks over the last 7 days sorted and grouped by module code.

. Test case: `tracker /view all` +
Expected: A histogram will be displayed featuring all of the tracked tasks sorted and grouped by module code.

. Test case: `tracker /view abc` +
Expected: Error displayed due to invalid period input.

==== Viewing List of Task Trackers
The `tracker /list` command syntax is as such: `tracker /list`.

. Prerequisites: List all trackers using `tracker /list` command. List must not be empty.

. Test case: `tracker /list` +
Expected: A list of all the task tracker entries made so far.
...
. Test case: `tracker /delete 1` +
Expected: The first task tracker is deleted from the list of Trackers.

. Test case: `tracker /delete 0` +
Expected: Error displayed due to invalid Task index.

. Other delete tracker commands to try: `tracker /delete abc`, `tracker /delete -1` +
Expected: Error displayed due to invalid input.
...
. Test case: `viewweek 32 12 2019` +
Expected: The `tasks` for the current week are shown. +
. Test case: `viewweek 30 12 2019` +
