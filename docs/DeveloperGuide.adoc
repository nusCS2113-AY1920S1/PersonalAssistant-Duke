...
==== Current Implementation
The current implementation of deleting `card` allow users to remove a credit card from their profile by entering
...
| *1.* Allowing deletion of credit cards by specifying the card name (case and trailing whitespace insensitive).
...
a| * Harder to implement because additional functions are required for trimming of spaces from card name and also
...
| *2.* Allowing deletion of credit cards by specifying the exact card name (case and trailing whitespace sensitive).
...
* User will be more prone to deleting a wrong card because card with same name but different cases may look alike.
...
a| * Unintuitive for user because they will be unable to remember a random 32 character alphanumeric string.

|======

After careful considering and weighing of pros and cons, we decided to use *approach 1*.

Although approach 1 is the hardest to implement, it is the most intuitive for users because they do not need to
memorise unnecessary order number, random 32 characters alphanumeric string, or the exact name. In addition, we have
decided for cards to have case insensitive names, so that users cannot add cards with same name but different cases.
Therefore, since card names are case insensitive , it make sense for users to expect case insensitive names
in the command.
