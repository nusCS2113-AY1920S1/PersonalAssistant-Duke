...
The filter mechanism in product is facilitated by `FilteredList` which wraps an `ObservableList` and filters using the provided Predicate.
...
The `ListProductCommand` will use this method to change the visibility of products with different status by passing in the corresponding predicate.
...
**Step1.** The user launches the application for the first time.
...
. Should be able to hold up to 80 orders without a noticeable sluggishness in performance for typical usage.
