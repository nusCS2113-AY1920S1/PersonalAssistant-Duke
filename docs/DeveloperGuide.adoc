...
.Activity diagram for the execute() method in RouteNodeNearbyCommand
image::SGTravel_routeNodeNearby_diagram.png[height="1290" width="850"]

The user can also automatically generate a new `Route` by using the `RouteGenerateCommand`. Given below is an example usage scenario and how the `Model`, `RouteList` and `PathFinder` object behave at each step.
