...
== Instructions for Manual Testing
=== Launch and Shutdown
Upon launch, a startup message should be shown including the logo, and a list of commands that the user
can use. This is list is available under UI.welcome() method and can be changed. +

Upon shutdown, a goodbye message is displayed before the app is terminated. If needed, the data/misterMusik.main.txt
file can be checked for discrepancies between what was stored in the program and what was written to the file for
storage.
