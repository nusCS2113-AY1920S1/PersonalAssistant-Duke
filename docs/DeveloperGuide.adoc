...
[width="100%",cols="22%,<60%,options="header",]
|=======================================================================
|Class | Functionality
|ArchDukeLogger| Handles logging of different levels.
...
|DateTimeHelper| Deals with date-related user inputs.
|MemberFactory| Parses and converts user input into a `Member` object.
|JsonConverter| Converts information stored by ArchDuke into Json files for persistent storage.
|ParserHelper| Contains methods used by other classes to parse the user input to retrieve the desired information.
|ProjectFactory| Parses and converts user input into a `Project` object.
|ReminderFactory| Parses and converts user input into a `Reminder` object.
|SortHelper|
|TaskFactory| Parses and converts user input into a `Task` object.
|ViewHelper| Formats and organises information in a presentable and user-friendly manner to display to the user.
|=======================================================================
