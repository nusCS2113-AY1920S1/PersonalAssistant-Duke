...
It
then calls
`ProductCommandParses` to parse the remaining information, i.e. "filter -scope archive".

**Step 4.** `ProductCommandParser` identifies that this is a `FilterProductCommand` through the word
"filter". It then calls `FilterProductCommandParser` to parse the scope.
...
**Step 7.** `UI` displays the response in `CommandResult`. In addition, `UI` is constantly listening for the
change in `Model`. Therefore UI will change to display archived products after model updates `filteredProducts`.
