...
The actions involving Degree-Related Tasks are performed mostly using the `DegreeTask` class. It extends the `Task` class, eventhough it does not use many methods from the aforementioned class. This was done in preparation for some proposed additional changes to this feature that will be discussed later in the document.



Currently, the `DegreeTask`
