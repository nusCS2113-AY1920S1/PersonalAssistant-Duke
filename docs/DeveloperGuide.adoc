...
*Step 3*: Duke will continue refer to Degrees stored in DegreeManager in order to determine where to get the degree information from. The csv file is expected to match the strings in the set.

*Step 4*: The degrees are created using the data from the csv files. It will then be linked via its key (the name of the Degree) to the Degree.

*Step 5*: If no errors occured, the map of degrees will be initialized, if not the maps and sets will be cleared and there will be a request to contact the System Adminstrator for assistance.
