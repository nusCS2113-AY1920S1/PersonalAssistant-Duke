...
Other alternatives would be to have the checks directly in the `GradeCommand` class, however this alternative was disregarded as other `Command` that had use for these checks were created, such as the `CapCommand` class.
...
[[Documentation]] 
== Documentation
We utilise AsciiDoc as the default to write our documentation, as the layout is more variable and aesthetically pleasing to view compared to other alternatives such as MarkDown. Additionally our preferred IDE IntelliJ has AsciiDoc support, and is thus easier for us to code and preview changes in.

[[Testing]] 
== Testing
=== Overview
We used JUnit testing to perform our testing.
=== Running Tests
(TBD)
