...
All of these commands are stored in a `CommandList` object, where it has methods to retrieve the previous `command` whenever undo is input, or to manually retrieve a particular command using `undoRedoPointer` whenever redo is input.
...
(Image showing an empty Command List)
...
(Image showing a CommandList with 1 element and the pointer at 0)
...
(Image showing a CommandList with 2 elements and the pointer at 1)
...
(Image showing a CommandList with 2 elements and the pointer at 0)
...
(Image showing a CommandList with 1 elements and the pointer at 0; show modCommand is removed)
