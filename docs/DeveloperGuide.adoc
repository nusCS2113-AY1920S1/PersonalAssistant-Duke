...
when the user inputs the "assign task" command. It first ensures that the input is valid but ensuring that there are sufficient
...
Step 5) If there are valid task numbers, a for loop is used to iterate through the tasks to handle the assignents
 ...
 ** Pros: More intuitive.
 ** Cons: Cyclic dependencies will exist between Task and Member objects, making it difficult to edit and change
assignments. This also results in increased coupling.
