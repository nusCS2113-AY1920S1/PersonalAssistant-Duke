...
** `SWITCH` -> `EMPTY`: `<Space>` or `<Newline>`, indicating the end of the switch name
* `EMPTY` 
** `EMPTY` -> `SWITCH`: `-`
...
** `ARG` -> `ARG`: any other character, which will be appended the argument being constructed

Preceding any transition character with a backslash `\` will escape it, allowing it to be treated as an ordinary character.

The initial state is `CMDARG`, as the first argument is always the argument of the `Command`, which is written to the `Command` by `setCmdArg()` when exiting the `CMDARG` state. After exiting the `CMDARG` state, it cannot be re-entered, as the argument, if any, would have been added already. The `checkArgAllowed()` and `checkSwitchAllowed()` functions are used to check if the respective type of input is permissible at that point when transitioning between `ARG` and `SWITCH`, or when transitioning from `EMPTY` to either. The check is not required for `CMDARG` as it only transitions to `SWITCH`, and only does so after calling `setCmdArg` - which will perform the necessary checks before setting the argument.

While in the `ARG`, `CMDARG` or `SWITCH` states, each character that is read is added to a StringBuilder `elementBuilder`. When exiting the state, the string is processed as a switch via `addSwitch()`,  saved as a switch argument by `writeElement()`, or as a command argument, as described above. For more details on how switches are processed, see above on `Command` objects, and on the <<Feature-Switch-Autocorrect,Switch Autocorrect>> feature.
