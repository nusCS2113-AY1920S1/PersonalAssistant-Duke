...
The `history` feature is implemented to keep track of the input commands entered by the user. It is facilitated by the `Storage` class which is called by the `Process` class in the `command` component.
...
The use case for the command `history` is given below to demonstrate how each component interacts with each other.

1. User executes the command `history` in the CLI. The input is passed from the `Ui` to `Command` where the input will be parsed to determine the command to execute.

2. The `Command` component will process the `history` command and load the text file consisting all the input commands from the `Storage` class into an arraylist.

3. The arraylist is then passed to `Ui` for printing the list of input commands to the user.
