...
=== 3.3. CRUD functionality for Task Assignments
...
Using a HashMap for storage allows for fast and easy writing and retrieval of data.
...
Step 2) User enters the command `assign task -i 1 2 -to 1 2 3 -rm 4`. This indicates the user would
like to assign tasks with index number 1 and 2 to members 1, 2 and 3, and also unassign/remove the task
from member 4.
...
of `ParserHelper.parseMembersIndexes()` and `ParserHelper.parseTasksIndexes()` [yet to refactor and implement].
