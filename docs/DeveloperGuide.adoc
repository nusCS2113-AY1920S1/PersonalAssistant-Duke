...
`./gradlew processResource` on *Mac/Linux* and `gradlew processResource` on *Windows*.
(On Mac/Linux:./gradlew processResource If you are getting bash error: `./gradlew: Permission denied` change
the permission of the shell script by executing `chmod 744 gradlew` in your terminal)
...
[discrete]
=== Use case: Generate report for a specific project (UC07)
