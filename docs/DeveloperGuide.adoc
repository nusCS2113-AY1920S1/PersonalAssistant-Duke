...
There are 3 parts of the input that must be parsed and validated. These are:

* task index numbers (1, 2)
* assignee indexes (1, 2, 3)
* unassignee indexes (4, 5)

There are 2 types of invalid inputs that could arise from assignment commands. The user should be informed about them in
the given order:

1. Invalid index numbers, such as non-integer values (eg. "abc"), or index numbers that do not yet exist
(eg. member index 999 in a project with 5 members).

2. Assigning a task that is already assigned to a member, or unassigning a task from members
who are not assigned in the first place.

Separating the assignment methods into a single, focused class can help to fulfil the
...
** Key: `Task`
** Value: ArrayList of `Member` objects (List of members assigned to task)
