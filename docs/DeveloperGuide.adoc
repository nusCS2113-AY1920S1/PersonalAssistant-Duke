...
The `execute` method of `TrackerCommand` class will read and save all `Tracker` objects stored in persistent storage, `tracker.csv` through the `readTrackerList` method in the `StorageManager` class.

* *Step 1a:* +
The `readTrackerList` method in `StorageManager` class will retrieve and read `tracker.csv` from persistent storage through the `loadFile` method in `Storage` class.
...
* *Step 1c:* +
The `processLine` method of `StorageParser` class will split each line into its respective fields through the use of `,` delimiters before parsing and assign them into the correct fields.
