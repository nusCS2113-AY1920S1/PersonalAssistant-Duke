...
** Pros: Easier to identify and undo/redo states once commands are known and saved. Much easier to implement than a partial state save. Common method in the industry to implement undo/redo. Easier to implement when new commands are added.
** Cons: Memory issues once too many commands are executed.
* Alternative 2: Saves a list in a history.
** Pros: Straightforward method to save previous states that can be called using undo/redo. No work needed when new commands are added.
** Cons: More work needed to support multiple types of lists. Memory issues once too many states are saved.
...
** Pros: Uses less memory than saving the state every time. Proper implementation of the memento pattern.
** Cons: A lot of work needed to "unexecute" every command possible. More commands means more "unexecution" is needed, and the workload scales higher than alternative 1.
