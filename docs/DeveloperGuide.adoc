...
.. Add some shows using the following format `add SHOW_NAME|SEAT_BASEPRICE|SHOWDATE1|...`. Input bye or close the window.
...
.. Test case: `sell Lion King | 29/2/2021 | A1 A2` +
   Expected: Display window does not change. Chat Box replies that it is an invalid date.

. Selling a seat to a non-existent show.

.. Test case: `sell Lion King | 11/10/2020 | A1 A2` +
   Expected: Display window does not change. Chat Box replies that show cannot be found.

[NOTE]
The following test cases should have some shows added before attempting. +
`add Lion King | 20 | 11/10/2020`

.. Test case: Wrong show name. `sell Lion Kingg | 11/10/2020 | A1 A2` +
   Expected: Display window does not change. Chat Box replies that the show cannot be found.

. Selling a non-existent seat.

.. Test case: All of the seats are non-existent. `sell Lion King | 11/10/2020 | A11 G1` +
   Expected: Display window changes to the seat display for the user input show. Chat Box replies that the seat does not exist.

.. Test case: Some of the seats can be purchased some are non-existent. `sell Lion King | 11/10/2020 | A1 F10 A11 G1`. +
   Expected: Seats that are purchased successfully changed to red. Chat Box displays seat number that are successfully purchased and the price followed by informing users which seats are non-existent.
