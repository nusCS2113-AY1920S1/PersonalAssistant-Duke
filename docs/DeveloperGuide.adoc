...
. *IntelliJ* IDE
[NOTE]
IntelliJ by default have Gradle and JavaFx plugins installed.
...
Allow users to add new `recurring Task` instead of selecting from existing `Tasks`. Allowing users to add new recurring tasks strongly overlaps with existing features and this increased coupling in the `OOF` program.
...
*Step 5.* +
The task is then added to calendarTask using the `addEntry()` method. After iterating through the current list of tasks, the `printViewWeek() method` in the `Ui` class is then called to print the tasks for the current week.

*Step 6.* +
In the `printViewWeek()` method, 3 main methods are being called to print the final output. Firstly, the `printViewWeekHeader()` method is called to print the header of the output which consists of the top border and the days of the current week.
...
[NOTE]
The lifeline of the User is a bar due to limitations in the software used to draw the diagram. Full details of the entire program are omitted to prevent the over-cluttering of the diagram.
...
*Step 1.* +
The user enters `free 30-10-2019`. The `parse` method in the `CommandParser` class is called to parse the input to obtain `30-10-2019` as the date to search for free time slots.
...
* Iterates through the current list of `Task` from the `TaskList` class and checks if an `Event` lies within the date given.
...
* Calls `sort` method in the `Collections` class to sort all `Event` start and end times in ascending order.
...
** Calling `printFreeSlot` method in the `Ui` class if `Event` does not coincide with a time slot.
...
visualising the amount of time spent on each `Module` filtered by `Day`, `Week` and `All`.
...
*Step 1:* +
The user enters the `start user guide`.
The `parse` method in `CommandParser` class is called to parse the user input to obtain the String `user guide` as the `description` that the user wants to start the Time Tracker timer for.
...
|18||* *|Paranoid University Student|Choose the threshold before the programs send an alert for me to complete my tasks|Can stay ahead of my schedule
...
. The user enters the description of the task.
...
* OOF detects empty date and time in the description of the task.
** OOF requests for the date and time of a task.
...
** OOF requests to confirm the decision.
** User confirms the decision.
** Use case ends if the User decides to cancel the action. Use case resumes from step 4 otherwise.
* At any time, the User chooses to re-enter the task description.
** OOF requests confirmation to re-enter the task description.
** User confirms to re-enter the task description.
...
. OOF requests for the index of the task to mark as complete.
. The user enters the index of the task to mark as complete.
...
* OOF detects the non-existent index of a task.
...
. The user wants to view tasks in a calendar format.
. OOF requests for a range of indexes of the tasks the user wishes to view in calendar format.
. The user enters the range of indexes of the task to view in the calendar format.
. OOF displays the tasks requested in the calendar format.

The use case ends.

*Extensions:*

* OOF detects the non-existent index of a task in the range.
** OOF requests for existent index and displays a range of indexes to choose from.
** User enters the required data.
** Use case resumes from step 4.
...
. The user wants to view a summary of the next day’s tasks.
. OOF requests for user input.
. The user enters the summary command.
...
. The user wants to add a task with date, start and end time.
. OOF requests for description, date, start and end time of the task.
. The user enters the requested details.
...
. The user chooses to activate the reminder for expiring tasks.
...
* At any time, the User chooses to cancel the activation.
** OOF requests to confirm the cancellation.
** User confirms the cancellation.
** Use case ends.
...
. OOF requests for confirmation of this action.
. The user confirms this request.
...
** Use case ends.
* At any time, the User chooses to cancel the request.
** OOF requests to confirm the cancellation.
** User confirms the cancellation.
** Use case ends.
...
. The user enters a description of the tasks.
...
** Use case resumes from step 4.
* At any time, the User chooses the stop finding tasks.
...
** Use case ends.
* At any time, the User chooses to cancel this action.
...
. OOF requests for the period from the User.
. The user enters the period of interest.
. OOF displays the free time slots within the period.

The use case ends.

*Extensions:*

* OOF detects that the period entered is invalid.
** OOF requests for the User to input a valid period.
** User enters a valid period.
** Steps 3a1-3a2 are repeated until a valid period is entered.
** Use case resumes from step 4.
* At any time, the User chooses to cancel the action.
** OOF requests for confirmation.
...
. The user chooses the task to be deleted.
. OOF deletes and displays the task that was deleted and the number of tasks saved in the program.
...
. The user enters the task and the postponed date.
...
. User requests to highlight overdue tasks.
. OOF requests to confirm the request.
. The user confirms the request.
...
. The user chooses to add recurring tasks.
. OOF displays the current tasks saved in the program and prompts the User to input the recurring task and its respective frequency.
. The user enters the task and recurring frequency.
