...
When a `SearchResult` object is constructed 

When an `ObjCommand` is executed, it first attempts to see if the object can be disambiguated without requesting for explicit intervention by the user. Although there are no constraints on how this is done, the typical `ObjCommand` allows user input in either index or string form. If the user did not input an index, the `ObjCommand` will typically search through the namespace of the parent through the `findByName()` methods in the different This is done by searching through the context and the objects in it.
