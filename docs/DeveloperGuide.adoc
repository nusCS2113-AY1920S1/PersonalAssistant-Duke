...
The Model component of the code is in charge of tracking and managing all of the data involved during the application's runtime. It accomplishes this by:
...
The Add feature aims to allow users to be able to quickly and efficiently add their meals to DIYeats. It accomplishes this by giving the user the ability to assign default nutritional values associated with certain meal names.
...
When the user enters a command associated with an add command, like: `add hotcakes /calories 300`, the application processes the command through the following steps:
...
.. The autocorrect function (implemented by: Foo Chi Hen) takes in the user input, and determines if there are any typos present in the user input.
...
.. This autocorrected command is then subjected to additional checks in the `AddItemCommandParser` class:
* Ensures that the command structure is correct.
...
. The `AddItemCommand#execute()` function then further invokes the `MealList#addStoredItem(...)` function, which assigns the new default values of `/calories 300` to a meal of name `hotcakes`
