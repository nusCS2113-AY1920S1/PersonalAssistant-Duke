...
[NOTE]
The `date` parameters `DD`, `MM` and `YYYY` are optional. If the `date` is not entered, the current week will be selected. Similarly, if there is an error in the `date` supplied, the feature will ignore the inputted `date` and print `tasks` for the current week.
