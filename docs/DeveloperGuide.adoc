...
The `Ui` component is resposible for: +
...
* User may `tracker /start` by  `taskListIndex` with a specific command.
* User may `tracker /stop` by  `taskListIndex` with a specific command.
* User may `tracker /pause` by  `taskListIndex` with a specific command.
* User may `tracker /view` by `day` with a specific command.
* User may `tracker /view` by `week` with a specific command.
* User may `tracker /view` by `all` with a specific command.
* User may `tracker /delete` by `taskListIndex` with a specific command.
* User may `tracker /list` with a specific command.
...
The `execute` method of `TrackerCommand` calls `processModuleTrackerList` method.
...
*Step 3:* +
If the user given instruction is `/list`, the `execute` method of `TrackerCommand` calls `printTrackerList` method in the `Ui` class.
...
[WARNING]
`InvalidArgumentException` will be thrown if the instruction given by the user is invalid.
`InvalidArgumentException` will be thrown if the saved `Tracker` object `description` do not match the `Task` object `description` where they are of the same `TaskList` index.

* *Step 6a:* +
The `updatedTrackerList` searches for the matching `Tracker` object in the ArrayList of `Tracker` objects before updating the `lastUpdated` and `startDate` with current `Date`.

* *Step 6b:* +
The `execute` method of `TrackerCommand` calls `writeTrackerList` method of `StorageManager` class to update `tracker.csv`.
...
* *Step 7b:* +
The `execute` method of `TrackerCommand` calls `setStatus` method in `Task` class to mark the `Task` as completed.
...
With the estimated ten work hours weekly on each module, this may result in hundreds of minutes spent on `Task` for each `moduleCode`.
...
Expected: An error would be displayed regarding adding of a semester that clashes with an existing semester. Using `semester /view` will not show a second copy of the semester added above.
...
Expected: Second semester is deleted from the list. Details of semester being deleted is echoed as an acknowledgement message.
...
==== Deleting a `module`
. Prerequisites: Semester has been selected using the `semester /select` command. List all module using the `module /view` command. At least 1 module is on the list.

. Test case: `module /delete 2` +
Expected: Second module is deleted from the list. Details of module being deleted is echoed as an acknowledgement message.
