...
==== Implementation
The `ViewTrackerCommand` class extends `Command` class by providing functions to display a histogram
...
Provided below is an example scenario of use and how `ViewTrackerCommand` class behaves and interact with other relevant classes.

*Step 1:* +
The user enters `start user guide`.
...
[WARNING]
`OofException` will be thrown is the `ModuleTrackerList` is empty.
...
** Alternatives Considered: +
The use of `.txt` and delimiter by `\t` has been considered.
...
* *Implementating of both `Tracker` and `TrackerList` as well as ModuleTracker` and `ModuleTrackerList` classes*
** Rationale: +
...
With the estimated amount of ten work hours weekly on each module, this may result in hundreds of minutes spent on `Assignment` for each `ModuleCode`.
