...
, and myself). It aims to assist undergraduates and fresh graduates to track and manage their finances
...
details of a specific transaction records. Don't worry *OwlMoney* allows you to find it with ease.
...
The command example allows the user to find and list all transaction records that matches the description `bubble tea`
...
[NOTE]
`findBankAccount` will throw an error if all bank accounts within the application fails the above checks.
